			...
			const itemsWithDetails = await Promise.all(
				menuItemIds.map(async (menuItemId) => {
					try {
						...
						// ✅ FIX 1: Get the full response object
						const response = await apiService.get(
							`/api/menu/${id}`
						);

						// ✅ FIX 2: Access the data inside response.data (cast to any to be safe)
						const menuItem = response.data as any;
						...
						console.error(
							`Failed to fetch menu item ${menuItemId}:`,
							error
						);
						return null;
					}
